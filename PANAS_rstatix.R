...
#to change scale of interes, change first term in pw-t-test to scale name 
#found in D1
PANAS_pwc <- D1 %>%
...
# ANOVA (rstatix)
PANAS.aov <- anova_test(data= D1, dv = panas_Positive_1, wid = D.subject_id,
                         between = D.randomization, within = D.redcap_event_name)
...
PANAS_line <- ggline(D1, x= 'D.redcap_event_name', y= 'panas_Positive_1',
                    ...
                    labs(subtitle = get_test_label(PANAS.aov, detailed = T))
