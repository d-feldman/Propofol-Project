...
##high doses
D_high_dose <- subset(D2, Dose_Time != 'Open_Label_High_Dose')
D_high_dose <- subset(D2, Dose_Time != 'Initial_Low_Dose')

D_high_dose$Dose_Time <- as.factor(D_high_dose$Dose_Time)
D_high_dose$Visit_Number <- as.factor(D_high_dose$Visit_Number)

D_high_dose$Dose <- D_high_dose$Dose_Time

###high dose first visit
D_high_dose_1 <- D_high_dose %>% filter( Visit_Number == 1)

D_high_dose_1$Patient <- c(24,25,27,34,37,39,41,45,46,50)

D_high_dose_1_comb <- D_high_dose_1[2:10 ,]
###high_dose_last visit
D_high_dose_6 <- D_high_dose %>% filter( Visit_Number == 6)

D_high_dose_6$Patient <- c(25,27,34,37,39,41,45,46,50)

###high dose 1st vs last visit

D_change_PANAS <- D_high_dose_1_comb$Patient
D_change_PANAS <- as.data.frame(D_change_PANAS)

D_change_PANAS$Sandess_Change <- D_high_dose_6$PANAS_Sadness_Difference - D_high_dose_1_comb$PANAS_Sadness_Difference
D_change_PANAS$Fatigue_Change <- D_high_dose_6$PANAS_Fatigue_Difference - D_high_dose_1_comb$PANAS_Fatigue_Difference

D_change_PANAS$Patient <- D_high_dose_6$Patient
