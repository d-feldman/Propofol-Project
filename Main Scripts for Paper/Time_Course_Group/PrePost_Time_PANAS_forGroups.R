...
D_pre$`Time Relative to Treatment` <- 'Pre Infusion'
D_pre <- D_pre %>% relocate(`Time Relative to Treatment`, .before= panas_Negative_Pre)

D_post$`Time Relative to Treatment` <- 'Post Infusion'
...
###grouping variable
D_results <- D_results %>% mutate(Dose_Time = case_when(Group == 'High' & Time == 'Pre Infusion'  ~ 'High_Dose_Pre', 
                                                        Group == 'Low'  & Time == 'Pre Infusion'  ~ 'Low_Dose_Pre', 
                                                        Group == 'High' & Time == 'Post Infusion' ~ 'High_Dose_Post',
                                                        Group == 'Low' & Time == 'Post Infusion'  ~ 'Low_Dose_Post'))

## Set Data Types
D_results$Visit_Number <- as.numeric(D_results$Visit_Number)
D_results$Time <- factor(D_results$Time, levels = c('Pre Infusion', 'Post Infusion'))
##Cleaning Done
##Ready for analysis